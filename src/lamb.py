...
from .utils import get_common_parameters, get_norm_bias_parameters, get_norm_parameters
...
class LAMB(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-6, weight_decay=0, bias_correction=False):
        if not 0.0 <= lr:
...
def create_lamb_optimizer(
    model, lr, betas=(0.9, 0.999), eps=1e-6, weight_decay=0, exclude_layers=["bn", "ln", "bias"], bias_correction=False
):
    ...
    if "bias" in exclude_layers:
        ...
        params = [
            dict(params=get_common_parameters(model, exclude_func=get_norm_parameters)),
            dict(params=get_norm_parameters(model), weight_decay=0),
        ]
    else:
        params = model.parameters()
    optimizer = LAMB(params, lr, betas=betas, eps=eps, weight_decay=weight_decay, bias_correction=bias_correction)
    return optimizer
